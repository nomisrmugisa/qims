...
const GenerateDataEntryForms = () => {
    ...
    useEffect(() => {
        const fetchProgramStages = async () => {
            try {
                setLoading(true);
                const response = await fetch(
                    `http://localhost:5002/api/programStages?fields=id,name&paging=false`,
                    {
                        headers: {
                            'Authorization': 'Basic ' + btoa('admin:5Am53808053@')
                        }
    ...
    useEffect(() => {
        const fetchDataElements = async () => {
            ...
            try {
                setLoading(true);
                const response = await fetch(
                    `http://localhost:5002/api/programStages/${selectedProgramStage}?fields=programStageDataElements[dataElement[id,name,code,valueType]]`,
                    {
                        headers: {
                            'Authorization': 'Basic ' + btoa('admin:5Am53808053@')
                        }
    ...
    useEffect(() => {
        const fetchProgramStageSections = async () => {
            try {
                const response = await fetch(
                    `http://localhost:5002/api/programStageSections.json?fields=id,name,programStage[id,name]&paging=false`,
                    {
                        headers: {
                            'Authorization': 'Basic ' + btoa('admin:5Am53808053@')
                        }
    ...
    useEffect(() => {
        const loadInitialForms = async () => {
            try {
                ...
                const response = await fetch(
                    `http://localhost:5002/api/dataStore/standard`,
                    {
                        headers: {
                            'Authorization': 'Basic ' + btoa('admin:5Am53808053@')
                        }
                ...
                const loadedForms = await Promise.all(
                    formKeys.map(async (key) => {
                        const formResponse = await fetch(
                            `http://localhost:5002/api/dataStore/standard/${key}`,
                            {
                                headers: {
                                    'Authorization': 'Basic ' + btoa('admin:5Am53808053@')
                                }
    ...
    const handleCreateNewDataElement = async () => {
        ...
        try {
            ...
            const createResponse = await fetch(`http://localhost:5002/api/dataElements`, {
                method: 'POST',
                headers: {
            ...
            if (selectedProgramStage) {
                ...
                const fullProgramStageResponse = await fetch(
                    `http://localhost:5002/api/programStages/${selectedProgramStage}.json?fields=id,name,program,programStageDataElements[dataElement[id],compulsory,sortOrder,allowProvidedElsewhere,displayInReports]`,
                    {
                        headers: {
                            'Authorization': 'Basic ' + btoa('admin:5Am53808053@')
                        }
                ...
                const updateResponse = await fetch(
                    `http://localhost:5002/api/programStages/${selectedProgramStage}`,
                    {
                        method: 'PUT',
                        headers: {
                            'Content-Type': 'application/json',
    ...
    const saveFormToDataStore = async (formKey, formData, isUpdate = false) => {
        try {
            const endpoint = `http://localhost:5002/api/dataStore/standard/${formKey}`;
    ...
    const loadFormFromDataStore = async (formKey) => {
        try {
            const response = await fetch(
                `http://localhost:5002/api/dataStore/standard/${formKey}`,
                {
                    headers: {
                        'Authorization': 'Basic ' + btoa('admin:5Am53808053@')
                    }
    ...
    const handleSaveForm = async () => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5002/api/metadata`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Basic ' + btoa('admin:5Am53808053@')
