...
const EditRequestForm = ({ request, onSave, onCancel }) => {
    ...
    const fetchLocationName = async (orgUnitId) => {
        ...
        try {
            const response = await fetch(`http://localhost:5002/api/organisationUnits/${orgUnitId}`, {
                headers: {
    ...
    const createOrgUnit = async (orgUnitId) => {
        try {
            ...
            const schemaResponse = await fetch(`http://localhost:5002/api/29/schemas/organisationUnit`, {
                method: 'POST',
                headers: {
            ...
            const orgUnitResponse = await fetch(`http://localhost:5002/api/29/organisationUnits`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Basic ' + btoa('admin:5Am53808053@')
    ...
    const addOrgUnitToProgram = async (orgUnitId) => {
        try {
            ...
            const results = await Promise.all(programs.map(async (programId) => {
                const response = await fetch(`http://localhost:5002/api/programs/${programId}/organisationUnits`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': 'Basic ' + btoa('admin:5Am53808053@')
    ...
    const createOrUpdateTEI = async (orgUnitId) => {
        try {
            ...
            if (!formData.tei) {
                ...
                response = await fetch(`http://localhost:5002/api/trackedEntityInstances`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': 'Basic ' + btoa('admin:5Am53808053@')
                ...
                // Update existing TEI
                response = await fetch(`http://localhost:5002/api/trackedEntityInstances/${formData.tei}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': 'Basic ' + btoa('admin:5Am53808053@')
    ...
    const createEnrollment = async (orgUnitId, programId, teiCalled) => {
        try {
            ...
            const response = await fetch(`http://localhost:5002/api/enrollments`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Basic ' + btoa('admin:5Am53808053@')
    ...
    const fetchOrgUnitUsersAssoc = async () => {
        try {
            const response = await fetch(
                `http://localhost:5002/api/users?filter=username:eq:${formData.employeeUsername}`,
                {
                    headers: {
                        'Authorization': 'Basic ' + btoa('admin:5Am53808053@')
                    }
    ...
    const enableUser = async (userId) => {
        try {
            const response = await fetch(
                `http://localhost:5002/api/40/users/${userId}`,
                {
                    method: 'PATCH',
                    headers: {
                        'Content-Type': 'application/json',
    ...
    const updateUserOrgUnits = async (userId, orgUnitUpdateType, newOrgUnitId) => {
        try {
            ...
            const assignResponse = await fetch(
                `http://localhost:5002/api/users/${userId}/${orgUnitUpdateType}/${newOrgUnitId}`,
                {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
            ...
            const deleteResponse = await fetch(
                `http://localhost:5002/api/users/${userId}/${orgUnitUpdateType}/OVpBNoteQ2Y`,
                {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
    ...
    const handleSubmit = async () => {
        try {
            ...
            setCurrentStep('Saving...');
            const API_URL = `http://localhost:5002/api/40/tracker?async=false&importStrategy=UPDATE`;
